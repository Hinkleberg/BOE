...
class MilitarySimAdapter:
    ...
    def _entity_record_to_dis_pdu(self, record) -> bytes:
        """
        Convert an EntityRecord or raw 64-byte entity record buffer to a
        serialised DIS Entity State PDU.
        In production, use your DIS library's PDU builder.
        This stub returns the record bytes prefixed with a 4-byte PDU type marker.
        """
        if isinstance(record, (bytes, bytearray)):
            ...
            if callable(to_bytes):
                ...
                payload = getattr(record, "__bytes__", None)
                if callable(payload):
                    payload = payload()
                else:
                    raise TypeError(
                        f"Cannot serialise EntityRecord of type {type(record).__name__}"
                    )

        PDU_TYPE_ENTITY_STATE = b'\x01\x00\x01\x01'  # stub header
        return PDU_TYPE_ENTITY_STATE + payload
