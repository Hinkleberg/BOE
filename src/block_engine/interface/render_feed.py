...
class RenderFeedServer:
    ...
    def notify_tiles_changed(
        self,
        tiles: List[int],
        world_reader: Callable[[int], bytes],
        entity_deltas: Optional[List[dict]] = None,
    ):
        # Called by external code when tiles changed or entity deltas arrive
        self.tick += 1
        for reader_writer, sub in list(self.subscriptions.items()):
            ...
            if not intersect and not entity_deltas:
                continue
            payloads = []
            for t in intersect:
            ...
            if entity_deltas:
                msg["entity_deltas"] = entity_deltas
            try:
