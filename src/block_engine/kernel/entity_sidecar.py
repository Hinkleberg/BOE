...
class EntitySidecar:
    ...
    def write_entity_raw(self, entity_id: int, raw_bytes: bytes) -> None:
        if entity_id == 0:
            raise ValueError("entity_id 0 is reserved for empty slots")
        if entity_id >= self._max:
            raise ValueError(f"entity_id {entity_id} exceeds max {self._max}")
        if len(raw_bytes) != ENTITY_SLOT_SIZE:
            raise ValueError(f"raw entity payload must be {ENTITY_SLOT_SIZE} bytes")
    ...
    def read_entity(self, entity_id: int) -> Optional[EntityRecord]:
        if entity_id == 0 or entity_id >= self._max:
            return None
        raw = self.read_entity_raw(entity_id)
        if raw is None:
            return None
        rec = EntityRecord.from_bytes(raw)
        return None if rec.is_empty else rec

    def read_entity_raw(self, entity_id: int) -> Optional[bytes]:
        if entity_id == 0 or entity_id >= self._max:
            return None
        offset = entity_id * ENTITY_SLOT_SIZE
        with self._lock:
            with open(self._path, "rb") as f:
                f.seek(offset)
                raw = f.read(ENTITY_SLOT_SIZE)
        return raw if len(raw) == ENTITY_SLOT_SIZE else None
