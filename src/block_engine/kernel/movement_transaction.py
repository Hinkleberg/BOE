...
class MovementTransaction:
    ...
    def commit(self, intent):
        ...
        self.sidecar.write_entity(rec)

        # Keep spatial membership and companion indices transaction-coupled.
        if hasattr(self.index, "upsert_entity"):
            self.index.upsert_entity(
                intent.entity_id,
                intent.x,
                intent.y,
                intent.z,
                entity_type=getattr(rec, "entity_type", None),
                owner_id=getattr(rec, "owner_id", None),
            )
        else:
            self.index.move(intent.entity_id, target)
